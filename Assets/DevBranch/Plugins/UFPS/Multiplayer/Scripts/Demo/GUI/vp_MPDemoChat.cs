			...
			else
			{
				switch(FilterMode)
				{
					...
					case MessageFilterBehavior.Silent:
						AddChatMessage(GetFormattedPlayerName(PhotonNetwork.player.ID) + m_InputLine, new PhotonMessageInfo(PhotonNetwork.player, Time.frameCount, photonView ));
	...
	public static void AddMessage(string message, bool broadcast = true)
	{
		...
		if (m_Chat != null)
		{
			if (broadcast && (PhotonNetwork.connectionStateDetailed == ClientState.Joined))
				...
				m_Chat.AddChatMessage(message, new PhotonMessageInfo(PhotonNetwork.player, Time.frameCount, m_Chat.photonView));
		}
